...
identity test. Dense scheduling and proxy hardening are automatic. The rail
policy is explicitly enabled by managed runtime configuration.
...
count changes. Collective algorithm, protocol, and tuner selection remain
upstream.
...
- A 512-rank, 1 GiB dense exchange completed three samples with zero validation
  errors at about `1.26 GB/s`. This release does not claim that the single-host
